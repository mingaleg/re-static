...
def main():
    ...
    errcount = 0
    errcount += run(["codespell", "--write-changes", *SRC_PATHS, *DEV_PATHS, *DOC_PATHS])
    errcount += run(["ruff", "check", "--fix", *SRC_PATHS, *DEV_PATHS])
    errcount += run(["ruff", "format", *SRC_PATHS, *DEV_PATHS])
